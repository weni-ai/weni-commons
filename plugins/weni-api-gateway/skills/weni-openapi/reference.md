...
`.cursor/rules/openapi-standards.mdc`. Read it when in doubt. The rules that
matter most here:
...
make sure every path appears exactly once in both.
...
3. **One schema or several.** Whether every service contributes to a single
   gateway schema or each publishes its own is undecided. Until it is settled,
   generate one file per service in this repository, which keeps both options
   open.
